...
class STLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=None, directional=0, name='STLSTM'):
        super(STLSTM, self).__init__()
        
        self.id = 'STLSTM'
        
        stlstm_hidden_size = hidden_size
        
        if output_size==None:
            output_size = input_size
            
        self.encoder = TimeDistributed(Encoder(input_size, hidden_size, output_size=hidden_size))
        self.decoder = TimeDistributed(Decoder(stlstm_hidden_size, hidden_size, output_size=output_size))

        self.encoder_stlstm = STLSTM_layers(hidden_size, stlstm_hidden_size, num_layers=3)
        self.decoder_stlstm = STLSTM_layers(stlstm_hidden_size, stlstm_hidden_size, num_layers=3)
...
class STLSTM_layers2(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=3, name='STLSTM-Layers'):
        super(STLSTM_layers2, self).__init__() 
    ...
    def forward(self, input, H=None, C=None, M=None, return_sequence=False, repeat_input=False, output_sequence_length=8, return_M=True):  
        # repeat input: input is not a sequence with dimensions [t,b,f,h,w], 
        # but only one step which is input for every recurrent loop [b,f,h,w]
        
        def getShapedTensor():
            if repeat_input:
                return torch.rand(input.size(0), self.hidden_size, input.size(2), input.size(3), dtype=input.dtype, device=input.device)
            else:
                return torch.rand(input.size(1), self.hidden_size, input.size(3), input.size(4), dtype=input.dtype, device=input.device)
             
        Hs = [getShapedTensor() for _ in range(len(self.cells))]
        Cs = [getShapedTensor() for _ in range(len(self.cells))]
        ...
        if return_sequence:
            if return_M:
                return torch.stack(Os), M
            return torch.stack(Os) # [t,b,hd,:,:]
        return Hs[-1], Cs[-1], M # [b,hd,:,:], [b,hd,:,:], [b,hd,:,:]

class STLSTM2(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=None, directional=0, name='STLSTM'):
        super(STLSTM2, self).__init__()
        ...
        self.encoder = TimeDistributed(
            nn.Sequential(
                nn.Conv2d(input_size,encoder_hidden_size,3, padding=(1,1)),
                nn.ReLU(),
                nn.BatchNorm2d(encoder_hidden_size),
                                
                nn.Conv2d(encoder_hidden_size,encoder_hidden_size,3, padding=(1,1), stride=(2,2)),
                nn.Sigmoid()
            )
        )
        
        if directional==0 or directional==1:
        
            self.encoder_clstm = STLSTM_layers(encoder_hidden_size,stlstm_hidden_size, num_layers=3)
            
        elif directional==2:
            self.encoder_clstm_fwd = STLSTM_layers(encoder_hidden_size,stlstm_hidden_size//2, num_layers=3)
            self.encoder_clstm_bwd = STLSTM_layers(encoder_hidden_size,stlstm_hidden_size//2, num_layers=3)
        
        self.decoder_clstm = STLSTM_layers(stlstm_hidden_size, stlstm_hidden_size, num_layers=3)
    
        self.decoder = TimeDistributed(   
            nn.Sequential(
                nn.Upsample(scale_factor=2),
                nn.Conv2d(stlstm_hidden_size,decoder_hidden_size,3, padding=(1,1)),
                nn.ReLU(),
                nn.BatchNorm2d(decoder_hidden_size),
                
                nn.Conv2d(decoder_hidden_size,output_size,3, padding=(1,1)),
                nn.Sigmoid()
            )
        )
    ...
    def forward(self, input, max_depth=10):
        ...
        ############# bidirectional part
        if self.directional==0 or self.directional==1:
            H, C, M = self.encoder_clstm(input)  
            
        elif self.directional==2:
            l = len(input)
            H, C, M = self.encoder_clstm_fwd(input[:l//2])  
            H_bwd, C_bwd, M_bwd = self.encoder_clstm_bwd(input.flip(1)[:l//2])  
            H, C, M = torch.cat([H, H_bwd], dim=1),  torch.cat([C, C_bwd], dim=1), torch.cat([M, M_bwd], dim=1)
            
        else:
            pass # assert input-error
        ###############
        
        input = self.decoder_clstm(H, H=H, C=C, M=M, return_sequence=True, repeat_input=True, return_M=False, output_sequence_length=max_depth)
        #print(input[0].shape)
        input = self.decoder(input)
...
if __name__=='__main__':
    ...
    x = torch.rand(3,8,1,120,120) # [Batch, Time, Deth, Height, Width]
